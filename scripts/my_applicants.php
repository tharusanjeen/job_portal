...
try {
    ...
    if (isset($_SESSION["job_provider_id"])) {
        ...
        // Query to fetch applications along with job and job seeker details
        $sql_application = "
            SELECT a.*, j.job_provider_email, j.job_title, j.company_name, j.job_location, 
                   ...
                   j.job_status, j.deadline, j.created_at, u.full_name AS job_seeker_name
            FROM applications a 
        ...
        if ($result_application->num_rows > 0) {
            while ($row = $result_application->fetch_assoc()) {
                // Combine application, job, and job seeker details
                $response[] = [
                    'application' => [
                        'id' => $row['id'],
                        'user_id' => $row['user_id'],
